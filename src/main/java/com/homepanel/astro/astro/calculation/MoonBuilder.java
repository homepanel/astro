...
public class MoonBuilder {
     ...
     */
    public Moon createMoon(Calendar calendar, Double latitude, Double longitude) {
    ...
    private void setMoonPhase(Calendar calendar, Moon moon) {
        ...
        long ageRangeTimeMillis = phase.getCalendarNew().getTimeInMillis() - parentNewMoonMillis;
        ...
        if (DateTimeUtils.isSameDay(calendar, phase.getCalendarNew())) {
            phase.setName(MoonPhaseName.NEW);
        } else if (DateTimeUtils.isSameDay(calendar, phase.getCalendarFirstQuarter())) {
            phase.setName(MoonPhaseName.FIRST_QUARTER);
        } else if (DateTimeUtils.isSameDay(calendar, phase.getCalendarThirdQuarter())) {
            phase.setName(MoonPhaseName.THIRD_QUARTER);
        } else if (DateTimeUtils.isSameDay(calendar, phase.getCalendarFull())) {
    ...
    private double CS(double x) {
        return Math.cos(x * SunBuilder.DEG2RAD);
    }

    private double SN(double x) {
        return Math.sin(x * SunBuilder.DEG2RAD);
    }
    ...
    private void setAzimuthElevationZodiac(double julianDate, double latitude, double longitude, Moon moon) {
        double lat = latitude * SunBuilder.DEG2RAD;
        double lon = longitude * SunBuilder.DEG2RAD;

        double gmst = toGMST(julianDate);
        double lmst = toLMST(gmst, lon) * 15. * SunBuilder.DEG2RAD;

        double d = julianDate - 2447891.5;
        double anomalyMean = 360 * SunBuilder.DEG2RAD / 365.242191 * d + 4.87650757829735 - 4.935239984568769;
        double nu = anomalyMean + 360.0 * SunBuilder.DEG2RAD / Math.PI * 0.016713 * Math.sin(anomalyMean);
        double sunLon = mod2Pi(nu + 4.935239984568769);

        double l0 = 318.351648 * SunBuilder.DEG2RAD;
        double p0 = 36.340410 * SunBuilder.DEG2RAD;
        double n0 = 318.510107 * SunBuilder.DEG2RAD;
        double i = 5.145396 * SunBuilder.DEG2RAD;
        double l = 13.1763966 * SunBuilder.DEG2RAD * d + l0;
        double mMoon = l - 0.1114041 * SunBuilder.DEG2RAD * d - p0;
        double n = n0 - 0.0529539 * SunBuilder.DEG2RAD * d;
        double c = l - sunLon;
        double ev = 1.2739 * SunBuilder.DEG2RAD * Math.sin(2 * c - mMoon);
        double ae = 0.1858 * SunBuilder.DEG2RAD * Math.sin(anomalyMean);
        double a3 = 0.37 * SunBuilder.DEG2RAD * Math.sin(anomalyMean);
        double mMoon2 = mMoon + ev - ae - a3;
        double ec = 6.2886 * SunBuilder.DEG2RAD * Math.sin(mMoon2);
        double a4 = 0.214 * SunBuilder.DEG2RAD * Math.sin(2 * mMoon2);
        double l2 = l + ev + ec - ae + a4;
        double v = 0.6583 * SunBuilder.DEG2RAD * Math.sin(2 * (l2 - sunLon));
        double l3 = l2 + v;
        double n2 = n - 0.16 * SunBuilder.DEG2RAD * Math.sin(anomalyMean);
        ...
        position.setAzimuth(azAlt[0] * SunBuilder.RAD2DEG);
        position.setElevation(azAlt[1] * SunBuilder.RAD2DEG + refraction(azAlt[1]));

        // zodiac
        double idxd = Math.floor(moonLon * SunBuilder.RAD2DEG / 30);
        int idx = 0;
        if (idxd < 0) {
    ...
    private double toLMST(double gmst, double lon) {
        return mod(gmst + SunBuilder.RAD2DEG * lon / 15., 24.);
    }
    ...
    private double refraction(double alt) {
        int pressure = 1015;
        int temperature = 10;
        double altdeg = alt * SunBuilder.RAD2DEG;
        ...
        for (int i = 0; i < 3; i++) {
            n = y + (7.31 / (y + 4.4));
            n = 1.0 / Math.tan(n * SunBuilder.DEG2RAD);
            d = n * p / (60.0 + q * (n + 39.0));
            n = y - y0;
            y0 = d - d0 - n;
